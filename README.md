...
PortBridge 是一个轻量、跨平台、本地私有化的 SSH 本地端口转发桌面客户端。它把常用的 `ssh -L localHost:localPort:remoteHost:remotePort` 工作流做成可视化配置，所有服务器、认证和映射配置都保存在用户自己的电脑上，不依赖云端账号，也不会把配置上传到第三方服务，让开发、测试、运维人员可以更安全地管理多台服务器和多条端口映射。
